...
def autonomous_short():
    ...
    while rear_distance.object_distance(MM) > 42: 
...
def driverControl():
    ...
    DrivetrainControl = Thread(drivev2)
    ...
    while True: 
        ...
        if controller_1.buttonLeft.pressing():
            ...
            spinner.stop()
        print(str(drivetrain_should_stop))
...
def userFeedback():
    ...
    while True:
        ...
        if controller_1.buttonB.pressing():# and f_time <10:
            expansion.set(True)
            cprint(2,"Expansion Launched")
            rumble(".")
            wait(5,SECONDS)
...
def drivev2():
    global drivetrain_should_stop
    while True:
        leftstick = controller_1.axis3.position()
        leftside = leftstick * leftstick *leftstick /10_000 
        rightstick = controller_1.axis2.position()
        rightside = rightstick * rightstick * rightstick / 10_000

        if not(drivetrain_should_stop):
            left_drive_smart.set_velocity(leftside,PERCENT)
            right_drive_smart.set_velocity(rightside,PERCENT)
